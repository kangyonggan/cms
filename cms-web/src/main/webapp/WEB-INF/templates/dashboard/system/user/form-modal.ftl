...
<@override name="modal-body">
    ...
    <input type="hidden" id="old-username" value="${user.username!''}"/>
        <@c.input name="user.username" label="用户名" readonly="${user.username???string('true', 'false')}"/>
        <@c.input name="user.realname" label="真实姓名"/>

        <#if !user.username??>
            <@c.input name="password" type="password" label="密码"/>
            <@c.input name="rePassword" type="password" label="确认密码"/>
        </#if>
