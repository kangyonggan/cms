...
<@override name="modal-body">
    <@c.form id="modal-form" action="${ctx}/dashboard/system/menu/${isEdit?string('update', 'save')}">
        <#if menu.id??>
        ...
        <@c.input name="name" value="${menu.name!''}" label="菜单名称" required="true"/>
        <@c.input name="code" value="${menu.code!''}" label="菜单代码" required="true" placeholder="格式参考:SYSTEM_USER"/>
        <#if menu.id??>
        <input type="hidden" id="old-code" value="${menu.code}"/>
        </#if>
        <@c.input name="url" value="${menu.url!''}" label="菜单地址" required="true" placeholder="格式参考:system/user"/>
        <@c.input name="sort" value="${menu.sort!'0'}" label="排序" required="true" placeholder="0排在最上面"/>
